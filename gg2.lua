--[[
    Grow A Garden 2 - Premium Script
    Generated by Antigravity (Google DeepMind Advanced Agentic Coding)
    UI Library: Dawid's Fluent UI (Premium Modern Aesthetic)
    Supports PC and Mobile (with draggable toggle button)
    
    FIXES APPLIED:
    - Disabled Acrylic Blur to resolve full background blur.
    - Moved loadModules to run asynchronously in the background so the UI renders INSTANTLY.
    - Added fallback values to dropdowns to prevent loading delays or crashes.
    - Dynamically update dropdown options at runtime once modules load via :SetValues().
    - Converted all GUI elements to use inline 'Callback' properties instead of ':OnChanged' calls for maximum compatibility.
    - Fixed CrateData.GetAllCrates iteration by removing pairs() wrapper.
    - Added Shovel Equip + Kill Aura.
    - Fixed Golden Seed Farm / Auto Collect.
]]
...
local function getStatsFile()
    ...
    local hasFolderSupport = makefolder and isfolder
    if hasFolderSupport then
        local success, exists = pcall(isfolder, folder)
        if success and not exists then
            pcall(makefolder, folder)
        end
        return folder .. "/" .. tostring(LocalPlayer.UserId) .. "_stats.json"
    else
        return "gg2_stats_" .. tostring(LocalPlayer.UserId) .. ".json"
    end
end
...
local statsFile = getStatsFile()
if readfile and isfile then
    local success, exists = pcall(isfile, statsFile)
    if success and exists then
        local readSuccess, content = pcall(readfile, statsFile)
...
stats.Executions = (stats.Executions or 0) + 1

if writefile then
    pcall(writefile, statsFile, HttpService:JSONEncode(stats))
end
...
-- Home Tab Paragraphs
local profileParagraph = Tabs.Home:AddParagraph({
    Title = "Player Profile",
    Content = "Loading profile..."
})

local sessionParagraph = Tabs.Home:AddParagraph({
    Title = "Script & Session Stats",
...
local function sendDiscordWebhook(webhookUrl, data)
    ...
    if not requestFunc then
        warn("No request function available for Discord webhook")
        return
    end
...
local function sendStatsToDiscord(timeString, sheckles, plotId, executor)
    local webhookUrl = "https://discord.com/api/webhooks/1515697407290118305/HYpkGIkDBu63Ha1OrH865rY-HcPsgj_ZtZybwTt7qITWfNN3SaetlHWcyuXzCM6LXX4X"
    local avatarUrl = string.format("https://www.roblox.com/headshot-thumbnail/image?userId=%d&width=150&height=150&format=png", LocalPlayer.UserId)
    
    local data = {
        ["embeds"] = {
            {
                ["title"] = "🌱 Grow A Garden 2 - Session Stats Log",
                ...
                ["fields"] = {
                    {
                        ["name"] = "👤 Player Profile",
                        ["value"] = string.format("**Username:** %s\n**Display Name:** %s\n**Account Age:** %d days", LocalPlayer.Name, LocalPlayer.DisplayName, LocalPlayer.AccountAge),
                        ["inline"] = true
                    },
                    {
                        ["name"] = "📊 Script & Session Stats",
                        ["value"] = string.format("**Executions:** %s\n**Session Playtime:** %s\n**Plot ID:** %s\n**Executor:** %s", tostring(stats.Executions), timeString, plotId, executor),
                        ["inline"] = true
                    },
                    {
                        ["name"] = "🎮 Game & Server Info",
                        ["value"] = string.format("**Place ID:** %d\n**Job ID:** %s\n**Sheckles:** %s", game.PlaceId, game.JobId, sheckles),
                        ["inline"] = false
                    }
                },
                ["timestamp"] = os.date("!%Y-%m-%dT%H:%M:%SZ"),
                ["footer"] = {
                    ["text"] = "Antigravity Logger • Grow A Garden 2"
                }
            }
        }
    }
...
task.spawn(function()
    while task.wait(1) do
        ...
        -- Get plot id
        local plotId = tostring(LocalPlayer:GetAttribute("PlotId") or "None")
        ...
        pcall(function()
            profileParagraph:SetContent(string.format("Username: %s\nDisplay Name: %s\nAccount Age: %d days", LocalPlayer.Name, LocalPlayer.DisplayName, LocalPlayer.AccountAge))
            
            sessionParagraph:SetContent(string.format("Script Executions: %s\nSession Playtime: %s\nPlot ID: %s\nExecutor: %s", tostring(stats.Executions), timeString, plotId, executor))
            
            gameParagraph:SetContent(string.format("Place ID: %s\nJob ID: %s\nSheckles: %s", tostring(game.PlaceId), tostring(game.JobId), sheckles))
        end)

        -- Discord Webhook Sending (Immediate execution, then every 5 minutes / 300 seconds)
        local now = os.time()
        if not lastWebhookSend or (now - lastWebhookSend >= 300) then
            lastWebhookSend = now
            task.spawn(sendStatsToDiscord, timeString, sheckles, plotId, executor)
        end
    end
end)
...
Fluent:Notify({
    Title = "Script Loaded",
    Content = "Grow A Garden 2 premium automation script has successfully initialized!",
    Duration = 5
})
