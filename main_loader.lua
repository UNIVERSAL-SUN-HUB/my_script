-- main_loader.lua
-- Paste this into your executor. No URL needed — loader.lua is fully embedded.
...
-- ──────────────────────────────────────────────
-- Embedded loader.lua code (no HTTP fetch required)
-- ──────────────────────────────────────────────
local LOADER_CODE = [=[
-- Loader UI Script
...
local function runLoadSequence()
    ...
    setProgress(0.10, "Initializing environment...", "Setting up player GUI and services")
    task.wait(0.5)

    setProgress(0.25, "Parsing embedded scripts...", "Reading 27 bundled script entries")
    task.wait(0.5)

    setProgress(0.45, "Building UI elements...", "Constructing menu frames, sidebar, tabs")
    task.wait(0.5)

    setProgress(0.62, "Creating script cards...", "Generating Execute buttons for each script")
    task.wait(0.45)

    setProgress(0.78, "Registering keybinds...", "LControl toggle, drag handler, confirm dialog")
    task.wait(0.4)

    setProgress(0.92, "Finalizing...", "Running pre-launch checks")
    task.wait(0.4)

    setProgress(1.0, "Done! Launching menu...", "All 27 scripts loaded successfully.")
    ...
    -- Execute the embedded loader code
    local fn, err = loadstring(LOADER_CODE)
    if fn then
        fn()
    else
        warn("[main_loader] Compile error: " .. tostring(err))
    end
end
